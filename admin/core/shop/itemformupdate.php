...
if ($w == "u") {
	alert("정상적으로 적용하였습니다.", EYOOM_ADMIN_URL . "/?dir=shop&amp;pid=itemform&amp;w=u&amp;it_id={$it_id}&amp;{$qstr}");
} else if ($w == "d")  {
    $qstr = "ca_id=$ca_id&amp;sfl=$sfl&amp;sca=$sca&amp;page=$page&amp;stx=".urlencode($stx)."&amp;save_stx=".urlencode($save_stx);
    alert("해당 상품을 삭제하였습니다.", EYOOM_ADMIN_URL , "/?dir=shop&amp;pid=itemlist&amp;$qstr");
}
...
<script>
    if (confirm("계속 입력하시겠습니까?"))
        location.href = "<?php echo EYOOM_ADMIN_URL . "/?dir=shop&pid=itemform&".str_replace('&amp;', '&', $qstr); ?>";
    else
        location.href = "<?php echo EYOOM_ADMIN_URL . "/?dir=shop&pid=itemlist&".str_replace('&amp;', '&', $qstr); ?>";
</script>
