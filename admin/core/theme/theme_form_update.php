...
// 이윰메뉴 생성
$g5_root = $eb->g5_root(str_replace("admin/index.php",'',$_SERVER['SCRIPT_NAME']));
...
if (file_exists($theme_sql_file)) {
	...
	sql_query("delete from {$g5['eyoom_ebslider']} where es_theme = '{$tm_name}' ");
	sql_query("delete from {$g5['eyoom_ebslider_item']} where ei_theme = '{$tm_name}' ");
	sql_query("delete from {$g5['eyoom_ebslider_ytitem']} where ei_theme = '{$tm_name}' ");
	sql_query("delete from {$g5['eyoom_ebcontents']} where ec_theme = '{$tm_name}' ");
	sql_query("delete from {$g5['eyoom_ebcontents_item']} where ci_theme = '{$tm_name}' ");
